...
class MessageListInner extends React.Component {
  ...
  getLastMessageOrGroup = () => {
    ...
    const lastMessageInGroup = lastElement?.querySelector(`[data-${prefix}-message]:last-of-type`)
    ...
    return (
      <div {...rest} className={classNames(cName, className)}>
        {loadingMore && (
          <div
            className={classNames(`${cName}__loading-more`, {
              [`${cName}__loading-more--bottom`]: loadingMorePosition === "bottom",
            })}
          >
            <Loader />
          </div>
        ...
        {typeof typingIndicator !== "undefined" && (
          <div className={`${cName}__typing-indicator-container`}>{typingIndicator}</div>
        )}
      </div>
    )
  }
