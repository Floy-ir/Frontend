...
const InfoContent = ({ info }) => <div className={`${cName}__info-content`}>{info}</div>

InfoContent.propTypes = {
  info: PropTypes.node,
}

export const ConversationContent = ({ lastSenderName, info, name, children, className, ...rest }) => {
        ...
        <>
          ...
          <div className={`${cName}__info`}>
            {typeofLastSenderName !== "undefined" ? (
              <>{typeofLastSenderName === "string" ? <LastSenderName name={lastSenderName} /> : lastSenderName} </>
            ) : null}
