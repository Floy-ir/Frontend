...
function StatusListInner({ className, children, size, selected, onChange = noop, itemsTabIndex, ...rest }, ref) {
  ...
  useImperativeHandle(ref, () => ({
    focus: (idx) => {
      ...
      const directChild = items.filter((item) => item.parentNode === listRef.current)
  ...
  return (
    <ul ref={listRef} {...rest} className={classNames(cName, className, { [`${cName}--${size}`]: size })}>
