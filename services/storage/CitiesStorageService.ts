...
export const CitiesStorageService = {
  getRecentSelections(): Array<{ value: string; label: string; code: string }> {
    return citiesStorage.getItem("recentSelections", [])
  },

  addRecentSelection(value: string, label: string, code: string, maxItems = 3): void {
    const current = this.getRecentSelections()
    const filtered = current.filter((item) => item.value !== value)
    const updated = [{ value, label, code }, ...filtered].slice(0, maxItems)
